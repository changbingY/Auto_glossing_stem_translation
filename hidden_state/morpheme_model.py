...
class MorphemeGlossingModel(LightningModule):
    ...
    def encode_trans_sentences(self, sentences: Tensor, sentence_lengths: Tensor) -> Tensor:
        ...
        return char_encodings[:,-1,:]

    def align_trans(self, translaton_encodings, num_morphemes_per_word, word_batch_mapping):
        ...
        aligned = []
        for batch_index in range(translaton_encodings.size()[0]):
            num_morphemes = num_morphemes_per_trans[batch_index]
            aligned.append(translaton_encodings[batch_index].expand(num_morphemes, -1))
    ...
    def forward(self, batch: Batch, training: bool = True):
        ...
        print(word_encodings.size())
        trans_char_encodings_h_n = self.encode_trans_sentences(
        ...
        print('morpheme_encodings.size()')
        print(morpheme_encodings.size())
        print('trans_char_encodings_h_n.size()')
        print(trans_char_encodings_h_n.size())
        #translation_word_encodings = torch.mean(translation_word_encodings, dim=(0,1)).unsqueeze(0)
        #trans_char_encodings_h_n = torch.mean(trans_char_encodings_h_n,dim=(0,1))
        trans_char_encodings_h_n = self.align_trans(trans_char_encodings_h_n, num_morphemes_per_word, batch.word_batch_mapping)
        print('after_mean_trans_char_encodings_h_n.size()')
        print(trans_char_encodings_h_n.size())
        combined_encodings = torch.cat(
            (morpheme_encodings,
            trans_char_encodings_h_n),dim=1
        )
