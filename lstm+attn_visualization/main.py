...
if __name__ == "__main__":
    ...
    early_stopping_callback = EarlyStopping(
        monitor="val_accuracy", patience=20, mode="max", verbose=True
    )
    ...
    if args.model == "ctc":
        ...
        model = MorphemeGlossingModel(
            source_alphabet_size=dm.source_alphabet_size,
            target_alphabet_size=dm.target_alphabet_size,
            translation_alphabet_size=dm.translation_alphabet_size,
            num_layers=args.layers,
            dropout=args.dropout,
            hidden_size=args.hidden,
            scheduler_gamma=args.gamma,
    ...
    translaton_attention_weights = []
    translations = []
    for batch_prediction in predictions:
        batch_prediction, batch_segmentation, batch_visualization_object = batch_prediction
        ...
        # ipdb.set_trace()
        translaton_attention_weights.extend(batch_visualization_object['translation_attention_weights'])
        translations.extend(batch_visualization_object['translations'])
        # batch_visualization_object = [(k, v) for k, v in batch_visualization_object.items()]
        # visualization_objects.extend(batch_visualization_object)

    decoded_predictions = []
    all_visualization_items = []
    for sentence_prediction, sentence_segmentation, translaton_attention_weight, translation in zip(
        sentence_predictions, sentence_segmentations, translaton_attention_weights, translations
    ):
        ...
        # ipdb.set_trace()
        items = {}
        assert len(translation) == translaton_attention_weight.size(1)
        items["translation"] = translation
        morphemes = []
        morpheme_prediction = [elem for word_prediction in decoded_sentence_prediction for elem in word_prediction]
        assert len(morpheme_prediction) == translaton_attention_weight.size(0)
        for i in range(len(morpheme_prediction)):
            morpheme = (morpheme_prediction[i], translaton_attention_weight[i].cpu().numpy().tolist())
            morphemes.append(morpheme)
        items["morphemes"] = morphemes
        all_visualization_items.append(items)
    ...
    fname = f"{args.language.lower()}_{args.model}_track{args.track}_visualization.json"
