...
class MorphemeGlossingModel(LightningModule):
    ...
    def encode_trans_sentences(self, sentences: List[str], sentence_lengths: Tensor, return_encoded_input: bool) -> Tensor:
        trans_bert_encodings, encoded_trans = self.bert_encoder(sentences, sentence_lengths, return_encoded_input)
        return trans_bert_encodings, encoded_trans  # tensor [batch_size, sequence_length, hidden_size]
    ...
    def align_words(self, word_encodings, num_morphemes_per_word):
        """ Align morphemes with words for attention computation
        Args:
            word_encodings: tensor [num_words, num_chars_per_word, hidden_size]
            num_morphemes_per_word: tensor [num_words]
        Return:
            aligned_word_encodings: tensor [num_morphemes, num_chars_per_word, hidden_size]
        """
        aligned_word_encodings = []
        for word_index, num_morphemes in enumerate(num_morphemes_per_word):
            temp_encodings = word_encodings[word_index].unsqueeze(0).repeat(num_morphemes, 1, 1)
            aligned_word_encodings.append(temp_encodings)
        aligned_word_encodings = torch.cat(aligned_word_encodings, dim=0)

        return aligned_word_encodings
    ...
    def mask_attention_weights(self, all_decoder_attn_weights, encoded_translations, num_morphemes_per_word, word_batch_mapping):
        """Mask attention weights.
        Args:
            all_decoder_attn_weights: [num_morphemes, max_morpheme_char_target_length, trans_sequence_length]
            encoded_translations: bert tokenized translation sentences
        Return:
            masked_attention_weights: list of tensor with size [num_morphemes_per_example, max_morpheme_char_target_length, trans_sequence_length]
        """
        num_morphemes_per_example = defaultdict(int)
        assert len(num_morphemes_per_word) == len(word_batch_mapping)
        for num_morphemes, example_index in zip(num_morphemes_per_word, word_batch_mapping):
        ...
        for example_index, num_morphemes in num_morphemes_per_example.items():
            # all_decoder_attn_weights[start: start + num_morphemes] --> apply mask and re-scale attention weights
            mask = encoded_translations["attention_mask"][example_index]
            temp_weights = all_decoder_attn_weights[start: start + num_morphemes]
            masked_weights = temp_weights.masked_fill(mask == 0, value=torch.tensor(-1e9))
            masked_weights = F.softmax(masked_weights, dim=-1).to("cpu")
            masked_attention_weights.append(masked_weights)
            start += num_morphemes
    ...
    def forward(self, batch: Batch, training: bool = True):
        ...
        trans_bert_encodings,encoded_trans = self.encode_trans_sentences(
            batch.trans_sentences_raw, batch.trans_sentence_lengths.cpu(),not training
        )
        # ipdb.set_trace()
        if self.classify_num_morphemes:
        ...
        # ipdb.set_trace()
        # Decoder
        trans_sequence_length = trans_bert_encodings.size(1)
        if training:
            ...
            all_decoder_attn_weights = Variable(torch.zeros(num_morphemes, max_morpheme_char_target_length - 1, trans_sequence_length))
        ...
        if USE_CUDA:
            ...
            all_decoder_attn_weights = all_decoder_attn_weights.cuda()
        # ipdb.set_trace()
        ...
        for t in range(max_morpheme_char_target_length - 1):
            decoder_output, decoder_hidden, decoder_attn = self.decoder(
                ...
                encoder_outputs=[aligned_word_encodings, aligned_trans_encodings]
            )

            all_decoder_outputs[:, t, :] = decoder_output
            all_decoder_attn_weights[:, t, :] = decoder_attn[1]
            if training:
                # ipdb.set_trace()
        ...
        if not training:
            all_decoder_attn_weights = self.mask_attention_weights(all_decoder_attn_weights, encoded_trans, num_morphemes_per_word, batch.word_batch_mapping)        
        ...
        return {
            ...
            "decoder_attn_weights": all_decoder_attn_weights,
            "encoded_translations": encoded_trans.to("cpu"),
        }
    ...
    def evaluation_step(self, batch: Batch):
        ...
        for predicted_idx, word_idx in zip(predicted_indices, morpheme_word_mapping):
            ...
            for idx in predicted_idx:
                if not finished and idx not in SPECIAL_TOKENS:
                    pred_idx.append(idx)
                else:
                    finished = True
    ...
    def get_word_segmentations(batch: Batch, best_path_matrix: Tensor):
        ...
        print(sentence_segmentations)
        return sentence_segmentations

    def align_decoder_outputs(self, all_decoder_outputs, num_morphemes_per_word, word_batch_mapping):
        """Align morpheme predictions with corresponding attentions weights on translation

        Args:
            all_decoder_outputs: prediction logits, [num_morphemes, max_morpheme_char_target_length, decoder_output_size/morpheme_vocab_size]
            num_morphemes_per_word: tensor [num_words]
            word_batch_mapping: list [num_words]
        Return:
            aligned_decoder_outputs: list of morphemes per example
        """
        num_morphemes_per_example = defaultdict(int)
        assert len(num_morphemes_per_word) == len(word_batch_mapping)
        for num_morphemes, example_index in zip(num_morphemes_per_word, word_batch_mapping):
        ...
        predicted_indices = torch.argmax(all_decoder_outputs, dim=-1).cpu().tolist()  # [num_morphemes, max_morpheme_char_target_length]
        predicted_morpheme_labels = []
        for predicted_idx in predicted_indices:
            ...
            for idx in predicted_idx:
                if not finished and idx not in SPECIAL_TOKENS:
                    ...
                    finished = True
            # predicted_word_labels[word_idx].append(pred_idx)
            predicted_morpheme_labels.append(pred_idx)  # [num_morphemes, morpheme_char_target_length]
        
        start = 0
        aligned_decoder_outputs = []
        for example_index, num_morphemes in num_morphemes_per_example.items():
            aligned_outputs = predicted_morpheme_labels[start: start + num_morphemes]
            aligned_decoder_outputs.append(aligned_outputs)
            start += num_morphemes
        return aligned_decoder_outputs
    ...
    def predict_step(self, batch: Batch, batch_idx: int, dataloader_idx: int = 0):
        ...
        aligned_decoder_outputs = self.align_decoder_outputs(scores["all_decoder_outputs"], scores["num_morphemes_per_word"], batch.word_batch_mapping)
        visualization_obj = {
            "attention_weights": scores["decoder_attn_weights"],
            "translations": scores["encoded_translations"],
            "decoder_outputs": aligned_decoder_outputs
        }

        # Print the learned segmentation

        return predicted_sentence_labels, learned_segmentation, visualization_obj
