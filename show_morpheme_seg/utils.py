...
#    """Detect if the input token is a whole unit.
#    """
#    text = re.sub(r'\.', '', text)
#    return text.isupper()

def is_uppercase(text: str):
    """Detect if the input token is a whole gloss unit.
    """
    #text = re.sub(r'\.', '', text)
    return text.isupper()
#def contain_uppercase(text: str):
#    """Detect if the input token contain the whole gloss unit.
#    """
#    rst = None
#    start_idx = 0
#    for item in text.split('.'):
#        if rst is None and is_uppercase(item):
#            rst = (item, start_idx, start_idx + len(item))
...
def contain_uppercase(text: str):
    ...
    for item in text.split('.'):
        if is_uppercase(item):
            rst.append((item, start_idx, start_idx + len(item)))
        start_idx += len(item) + 1
    return rst
