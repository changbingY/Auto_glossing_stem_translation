...
class MorphemeGlossingModel(LightningModule):
       ...
       # translation_word_encodings = torch.mean(translation_word_encodings, dim=(0,1)).unsqueeze(0)
        ...
        for batch_index in morpheme_encodings_per_batch.keys():
            ...
            for each_morpheme_encoding in morpheme_encodings_this_batch:
                ...
                translation_attention_weights_this_batch.append(attn_score.squeeze(1))
                # ipdb.set_trace()
            ...
            translation_attention_weights_this_batch = torch.stack(translation_attention_weights_this_batch)
            
            translation_attention_encodings.append(translation_attention_encodings_this_batch)
            translation_attention_weights.append(translation_attention_weights_this_batch)
        ...
        return {
            ...
            "translation_attention_weights": translation_attention_weights,
            "translations": batch.trans_sentences_raw,
        }
    ...
    # def align_translation_attention_weights(self, translation_attention_weights, num_morphemes_per_word, word_batch_mapping):
    #     """Align translation attention weights.
    #     Args:
    #         translation_attention_weights: list of attention weights with size batch_size and element list of size num_morphemes_per_example and element tensor [trans_sequence_length,]
    #         num_morphemes_per_word: tensor [num_words]
    #         word_batch_mapping: list [num_words]
    #     Return:
    #         aligned_attention_weights: list of length batch_size and each element tensor [num_morphemes_per_example, trans_sequence_length]
    #     """
    ...
    def predict_step(self, batch: Batch, batch_idx: int, dataloader_idx: int = 0):
        ...
        visualization_object = {
            "translation_attention_weights": scores["translation_attention_weights"],
            "translations": scores["translations"]
        }

        return predicted_sentence_labels, learned_segmentation, visualization_object
