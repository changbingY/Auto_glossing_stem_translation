...
from utils import nlp_pad_sequence, is_uppercase, contain_uppercase
...
def _check_datapoint(datapoint: RawDatapoint) -> bool:
    source, translation, target, morphemes = (
        datapoint["source"],
    ...
    if translation is None:
        raise ValueError("Found Datapoint without translation.")
...
def read_glossing_file(file) -> GlossingFileData:
    ...
    # Return Data
    return GlossingFileData(sources=sources, targets=targets, morphemes=morphemes, translations=translations)
...
def _make_translation_sentence(
    translation: List[str], sos_token: str = "[SOS]", eos_token: str = "[EOS]"
) -> List[str]:
    return [sos_token] + list(" ".join(translation)) + [eos_token]

#def _make_target_char_sentence(
#    target: List[str], sos_token: str = "[SOS]", eos_token: str = "[EOS]"
#) -> List[str]:
#    if is_uppercase(target):
#        return [sos_token] + [target] + [eos_token]
#    else:
#        return [sos_token] + list(target) + [eos_token]
 #   #return [sos_token] + list(target) + [eos_token]
...
#        return [sos_token] + [target] + [eos_token]
#    else:
#        rst = contain_uppercase(target)
#        if rst is not None:
#            #ipdb.set_trace()
#            return [sos_token] + list(target[:rst[1]]) + [target[rst[1]:rst[2]]] + list(target[rst[2]:]) + [eos_token]
#        else:
#            return [sos_token] + list(target) + [eos_token]
...
def _make_target_char_sentence(
    ...
    if rst:
        ...
        start = 0
        for item in rst:
            ret.extend(list(target[start:item[1]]))
            ret.append(target[item[1]:item[2]])
            start = item[2]
        ret.extend(list(target[start:]))
        return [sos_token] + ret + [eos_token]
    else:
        return [sos_token] + list(target) + [eos_token]
...
def _batch_collate(
    ...
    translation_tokenizer: Vocab,
    target_tokenizer: Vocab,
    target_char_tokenizer: Vocab,
    sos_token: str = "[SOS]",
    eos_token: str = "[EOS]",
    sot_token: str = "[SOS]",
    eot_token: str = "[EOS]",
    ...
    translation_sentences_raw = [" ".join(translation) for translation in translations]
    ...
    if all(target is not None for target in targets):
        ...
        raw_morpheme_targets = list(chain.from_iterable(raw_word_targets))
        make_target_char_sentence = partial(
            _make_target_char_sentence, sos_token=sos_token, eos_token=eos_token
        )
        target_char_sentences = [make_target_char_sentence(target) for target in raw_morpheme_targets]
        morpheme_char_targets = [target_char_tokenizer(target) for target in target_char_sentences]
        morpheme_char_target_tensors = indices_to_tensor(morpheme_char_targets)
        morpheme_char_target_lengths = torch.tensor(
            [len(target) for target in morpheme_char_targets]
        ).long()
        ...
        raw_morpheme_targets = None
        morpheme_char_target_tensors = None
        morpheme_char_target_lengths = None
    ...
    return Batch(
        ...
        trans_sentences_raw = translation_sentences_raw,
        ...
        raw_morphemes=raw_morphemes,
        raw_morpheme_targets=raw_morpheme_targets,
        morpheme_char_targets=morpheme_char_target_tensors,
        morpheme_char_target_lengths=morpheme_char_target_lengths,
    )
...
class GlossingDataset(LightningDataModule):
    ...
    def setup(self, stage: str) -> None:
        if stage == "fit" or stage is None:
            ...
            #self.target_char_alphabet = set()
            #for target in train_data.targets:
            #    for word_labels in target:
            #        for word_label in word_labels:
            #            #self.target_char_alphabet.update(set(word_label))
            #            if is_uppercase(word_label):
            #                self.target_char_alphabet.add(word_label)
            #            else:
            #                self.target_char_alphabet.update(set(word_label))
            #self.target_char_alphabet = list(sorted(self.target_char_alphabet))
            self.target_char_alphabet = set()
            for target in train_data.targets:
                for word_labels in target:
                    for word_label in word_labels:
                        rst = contain_uppercase(word_label)
                        if rst:
                            start = 0
                            for item in rst:
                                self.target_char_alphabet.update(set(word_label[start:item[1]]))
                                self.target_char_alphabet.add(word_label[item[1]:item[2]])
                                start = item[2]
                            self.target_char_alphabet.update(set(word_label[start:]))
                        else:
                            self.target_char_alphabet.update(set(word_label))
                        #if is_uppercase(word_label):
                        #    self.target_char_alphabet.add(word_label)
                        ...
                        #    rst = contain_uppercase(word_label)
                        #    if rst is not None:
                        #        self.target_char_alphabet.update(set(word_label[:rst[1]]))
                        #        self.target_char_alphabet.add(word_label[rst[1]:rst[2]])
                        #        self.target_char_alphabet.update(set(word_label[rst[2]:]))
                         #   else:
                         #       self.target_char_alphabet.update(set(word_label))
            self.target_char_alphabet = list(sorted(self.target_char_alphabet))
            ...
            self.target_char_alphabet_size = len(self.target_char_alphabet) + 4
            ...
            self.target_char_tokenizer = build_vocab_from_iterator(
                [[symbol] for symbol in self.target_char_alphabet],
                specials=self.special_tokens
            )
            ...
            self.target_char_tokenizer.set_default_index(1)
            ...
            self._batch_collate = partial(
                ...
                target_char_tokenizer=self.target_char_tokenizer,
            )
